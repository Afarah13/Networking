...
with hysteresis by using a closed-form solution.  Finally in  \cite{ait2015threshold}, the authors analyze the system through stochastic
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=0.47\textwidth, height=0.22\textwidth,]{images/exemple_par_bloc2.png}
...
\label{sec:ResModel}
In order to compute the exact performance measures of the presented models, we  expose hereafter some techniques to solve the CTMC and compute the steady state probability vector. In this section, we present three resolution methods. A comparison in terms of execution times of these methods is presented in Section~\ref{sec:ResNum}.
...
So, given the $\{X_{t}\}_{t \geq 0}$ Markov chain with state space $A$. We partition the state space  $A$ into $K$ distinct sets denoted $A_{k}$, where:
\begin{equation}
...
\noindent Then using equation \ref{eq8}, and equation \ref{eq7}, in equation \ref{eq5}, we deduce for $R_1+1 \leq m \leq  F_1$,  equation
(\ref{eq1c}).
%that
...
for the model presented in this paper.  Once the steady state vector calculated, we can deduce various performance measures.  Expressed as an expected Markov reward
function $\cal{R}$, where $\cal{R}$ $=\sum_{i,j}\pi(i,\,j) \,r(i,\,j)$  and $r(i,\,j)$ the reward of state $(i,\,j)$, the performance metrics of interest are described hereafter.
