...
%According to the occupation of  the queue and the thresholds, the VMs are activated  and deactivated by block. 
...
In the sequel, we give the algorithm which compute $\pi$. 
...
The relevance of our work is that we take more general cases for the thresholds  for each level $2 \leq k \leq K$ : $R_{k} \leq F_{k-1}$, and  $R_{k} > F_{k-1}$.
